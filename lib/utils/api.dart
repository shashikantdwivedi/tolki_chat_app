...
abstract class XApi {
  static post({endpoint, data, headers}) async {
    var url = Uri.https(XConstants.apiDomain, 'apis/v1/core$endpoint');
    log(url.toString());
  ...
  static get({endpoint, headers, params, showAlert=true}) async {
    var url =
        Uri.https(XConstants.apiDomain, 'apis/v1/core$endpoint', params ?? {});
    log(url.toString());
  ...
  static upload({endpoint, headers, files}) async {
    var request = http.MultipartRequest("POST",
        Uri.parse('https://${XConstants.apiDomain}/apis/v1/core$endpoint'));
